...
class Pattern:
    ...
    def get_consensus(self, kind='seq'):
        ...
        """
        max_idx = np.array(self._get_track(kind)).argmax(axis=1)
        ...
        if possible_names == ['fwd', 'rev']:
            ...
            if len(contrib_name) == 1:
                ...
                if(len(possible_names) == 1):
                    ...
                    #We have several subgroups, like MPRA and counts
                    contrib_names = [pn for pn in possible_names if "contrib_scores" in pn][0]
                    hyp_contrib_names = [pn for pn in possible_names if "hypothetical_contribs" in pn][0]
                    return Pattern(name,
                                   seq=grp['sequence']['fwd'],
                                   contrib={t: grp[t][contrib_names]['fwd'][:] for t in tasks},
                                   hyp_contrib={t: grp[t][hyp_contrib_names]['fwd'][:] for t in tasks})
